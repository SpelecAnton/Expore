// engine.js v7.9
// Changelog:
// v7.9 — Bob phase is now driven by actual horizontal displacement (dx,dz)
//        instead of raw time. Fixes two bugs:
//          • Walking into a wall: keys held but no movement → phase stays
//            frozen, bob fades out naturally.
//          • Walking on a slope: actual horizontal progress is slower →
//            bob rate matches perceived stride instead of running ahead.
//        bobSpeed unit changed: now radians per world unit traveled
//        (was rad/s). New default is 2.0, which reproduces the same
//        feel as the old default of 10 at full MOVE_SPEED (5.6 u/s).
...
export async function initEngine({
    ...
    // ── Bob state ─────────────────────────────────────────────────────────────
    // _bobPhase:  accumulated sine phase (radians); advances only while walking
    // _bobFactor: smooth 0→1 blend weight; fades in when starting to walk and
    //             fades out when stopping, preventing a jarring pop
    let _bobPhase  = 0;
    let _bobFactor = 0;
    ...
    (function loop() {
        ...
        // 6. Camera bob ───────────────────────────────────────────────────────
        //    Active when on the ground AND actually moving horizontally.
        //    Using measured displacement (not key state) means:
        //      - walking into a wall → no movement → bob pauses
        //      - going up a slope   → less horizontal progress → slower bob
        const bobActive = bobStrength > 0 && physics.isOnGround && horizDist > 0.0001;

        //    Smooth the factor so bob fades in/out instead of popping.
        //    Factor 8 → ~125 ms ramp time.
        _bobFactor += ((bobActive ? 1.0 : 0.0) - _bobFactor) * Math.min(1.0, dt * 8.0);

        //    Advance phase by actual distance traveled × bobSpeed (rad/unit).
        //    The phase only grows when the player actually moves, so bob
        //    frequency is always proportional to real stride length.
        if (bobActive) _bobPhase += horizDist * bobSpeed;

        //    Compute vertical offset: sin wave scaled by strength × blend factor
        const bobOffset = Math.sin(_bobPhase) * bobStrength * _bobFactor;

        //    Apply offset to camera Y for this render call ONLY.
        //    Save physics Y first so we can fully restore it afterward —
        //    Rapier reads cam.position on the next physics.update() call and
        //    must never see the bob displacement.
        const physicsY    = cam.position.y;
        cam.position.y   += bobOffset;

        // 7. Render
        if (sceneReady) composer.render();
        else            renderer.render(scene, cam);

        // 8. Restore physics Y — Rapier gets the correct position next frame
        cam.position.y = physicsY;
    })();
}
