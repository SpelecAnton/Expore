// engine.js v7.13
// Changelog:
// v7.13 — Three separate root-cause fixes for bob amplitude inconsistency:
//         (1) SLOPE: bob felt bigger uphill, smaller downhill because the
//             world-Y sine offset interacts with a tilted floor — the floor
//             is perceptually closer going up and farther going down.
//             Fix: scale bobOffset by slopeScale = horizDist/totalDist3D
//             (= cos(slope_angle)). On flat ground slopeScale=1; on a 30°
//             slope it's 0.87, compensating exactly for the geometry.
//             slopeScale is lerped (K=10) to avoid pop at slope transitions.
//         (2) SPACE with jump_speed=0: each frame R is set false, gravity
//             accumulates u.y=-GRAVITY*dt, KCC sends a tiny downward vector
//             into Rapier which produces sub-millimetre Y noise. Fixed in
//             physics.js v1.2 by guarding jump with JUMP_SPEED>0.
//         (3) _renderY velocity-follower from v7.12 removed — it added
//             unnecessary complexity and didn't solve the perceptual issue.
// v7.12 — Fix "up stronger, down weaker" bob asymmetry introduced by v7.10.
...
export async function initEngine({
    ...
    let _bobPhase=0,_bobFactor=0,_slopeScale=1.0;
    ...
    (function loop() {
        ...
        const prevX=cam.position.x,prevY=cam.position.y,prevZ=cam.position.z;
        ...
        // slopeScale = cos(slope_angle) = horizDist/totalDist3D.
        // Reduces world-Y bob amplitude proportionally to slope steepness so
        // the perceptual amplitude (relative to tilted floor) stays constant.
        const dy=cam.position.y-prevY;
        const t3d=Math.sqrt(horizDist*horizDist+dy*dy);
        const tgtSlope=t3d>1e-5?horizDist/t3d:1.0;
        _slopeScale+=(tgtSlope-_slopeScale)*(1-Math.exp(-dt*10));
        ...
        const bobOffset=Math.sin(_bobPhase)*bobStrength*_bobFactor*_slopeScale;
