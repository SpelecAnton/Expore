 ...
 *   Shows only messages from the last 60 seconds; older ones vanish automatically.
 *
...
const PING_IV    = 5000;
const TICKER_IV  = 8000;   // re-render ticker every N ms to expire old messages
const RECENT_SEC = 60;     // seconds a message stays in the ticker
...
export function createChatOverlay({
  ...
  const _nickInput   = _el('input', { id: 'co-nick',      class: 'co-input',
                            maxlength: '32', placeholder: 'Nick',       autocomplete: 'off', spellcheck: 'false' });
  ...
  const _tabH = e => {
    if (e.key === 'Tab') {
      e.preventDefault();
      e.stopImmediatePropagation(); // also blocks other capture-phase listeners
      _setOpen(!_isOpen);
    }
  };
  ...
  function _refreshTicker() {
    if (_isOpen) return;                              // ticker hidden when panel is open
    const cutoff = Date.now() / 1000 - RECENT_SEC;
    const recent = _allMsgs.filter(m => m.timestamp > cutoff).slice(-5);
    ...
    for (const msg of recent) {
      ...
      if (msg.text) {
        // Decode PHP HTML-escape then show as plain text
  ...
  function _updatePlayerStrip() {
    ...
    for (const p of players) {
      ...
      const name = document.createElement('span');  name.textContent = p.nick; // safe: textContent
  ...
  function _renderMsg(msg) {
    ...
    // Text — msg.text is PHP htmlspecialchars-escaped, safe for innerHTML after linkify
    if (msg.text) {
